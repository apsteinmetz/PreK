...
Select only those pages that are directory listings and split into lines.  The `pdf_text` function splits each page into a list item of text.  To identify those pages which have listings we chose a term that is unlikely to appear elsewhere in the document.  I chose "Playspace."  It only appears as a false positive once in the interpretive key. Here's what a single entry looks like in the PDF file:
...
To create the legend we 'simply' create a 3x3 tile plot matching the fill colors in the map, label the axes appropriately and save the image.  Then, using `addcontrol(), shove it into a corner of the map.
