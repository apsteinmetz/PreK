...
We show some editorial judgement in setting the levels of the interplay of income and seats.  This is, for our purposes, high income and low seats is "good" along with "low income" and "high seats."  We will use this ordering when coloring our maps.
...
bins<-3
all_data_2018 <- all_data_2018 %>% 
  mutate(seatsBin=cut2(seatsPer100Kids,g=bins,levels.mean = TRUE)) %>% 
  mutate(incomeBin=cut2(HouseholdIncome,g=bins,levels.mean = TRUE))
#recode numeric bins to descriptions
# judge what are good and bad combos of income and seats
desired_combos <- c("Low Many",
                    "High Few",
                    "Low Average",
                    "Average Many",
                    "Average Average",
...
all_data_2018 <- all_data_2018 %>% 
  mutate(combo = factor(paste(seatsBin,incomeBin),
                           levels = desired_combos))
...
bvscale <- scale_fill_manual(name="", values=bvColors, drop=FALSE)

all_data_2018 %>%
  ggplot(aes(fill = combo, color = combo)) +
  geom_sf()+
