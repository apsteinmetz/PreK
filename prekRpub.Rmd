...
Thanks to Joshua Stevens for the inspiration and color theory of bi-variate maps (http://www.joshuastevens.net/cartography/make-a-bivariate-choropleth-map/). Thanks to Ari Lamstein for the awesome suite of choropleth packages (http://www.arilamstein.com/).  Finally, thanks to the cool kids at RStudio.com who have ramped up the R ecosystem in amazing ways!

One complication is to directly grab the income and population data from the census bureau requires an API key.  You'll have to get your own here: http://api.census.gov/data/key_signup.html.

Also note that this code assumes NYC keeps the pre-K directories at the same URLs.
...
Requires an API key.  Download household median income, population and number of kids under 3 by Zip code
...
Now that we have converted all the PDFs into text and joined them together we pull out the Zip, seat count and day length of each school.  Note the pretty heroic (for me, anyway) regular expression, "pkseattokens.""
...
```
So we go from this image grab of a PDF booklet:
```{r}
# This jpeg is not created within the scope of this notebook.  It's just for show.
myjpg <- readJPEG('prekbooklet.jpg')
plot(1:2,type='n',xaxt='n',yaxt='n',xlab='',ylab='')
rasterImage(myjpg,1,1,2.0,1.5)
```
