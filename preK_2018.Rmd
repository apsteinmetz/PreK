...
First, is the information about the pre-K program from the city.  We are mainly interested in the number of seats by zip code. To get this we have to download a separate directory for each borough in PDF format and scrape each one.
...
Finally, we need geographic information about zip code boundaries.  The "Tigris" data at the census is now available in `sf` format thanks to the `tigris` package.

The cup of the geospatial analyst using R really runneth over!
...
cool bi-variate choropleth maps! I had to invent a novel method in R to get a good looking bivariate legend onto the chart.
...
Extract text from PDFs.  Select only those pages that are directory listings and split into lines.  The `pdf_text` function splits each page into a list item of text.  To identify those pages which have listings we chose a term that is unlikely to appear elsewhere in the document.  I chose "Playspace."  It donly appears as a false positivee once in a facsimile of a listing in the interpretive guide.
