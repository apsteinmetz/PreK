...
census_api_key(Sys.getenv("CENSUS_KEY"),overwrite = TRUE)
...
```{r}
load("~/R/NYCPreK/data/schools_2018.rdata")
load("~/R/NYCPreK/data/alldata2018.rdata")
load("~/R/NYCPreK/data/acs_dat.rdata")
load("~/R/NYCPreK/data/geo_nyc_zips.rdata")
...
all_data_2018 <- all_data_2018 %>% 
  mutate(combo = factor(paste(incomeBin,seatsBin),
                        levels = ordered_combos))

popup_html<-popupTable(all_data_2018,
           zcol=c("combo","HouseholdIncome","numSeats","zip"),
           row.numbers = FALSE) %>% 
  # this does comma formatting of the HTML string.  Thanks, Stack Overflow!
