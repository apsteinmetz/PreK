...
mapview(all_data_2018,zcol=c("IncAndSeats"),
        popup = popup_html,
...
mv <-mv %>% addLogo("img/bv_legend.png",src = "local",position = "topleft",width=100,height=100,offset.x = 70)
mv
```
...
mv_2016 <- mapview(all_data_2016,zcol=c("IncAndSeats"),
        ...
        legend = FALSE) %>% 
    addLogo("img/bv_legend.png",src = "local",position = "topleft",width=100,height=100,offset.x = 70) %>%
    {.}


all_data_2018 <- all_data_2018 %>% mutate(seatsChange16to18=numSeats - all_data_2016$numSeats)

mv_delta<- mapview(all_data_2018,zcol=c("seatsChange16to18"),
                   popup = popup_html,
                   col.regions = viridis::plasma(10),
                   map.types="OpenStreetMap.BlackAndWhite",
                   layer.name = "Change in Seats 2016-2016") %>% 
  {.}
