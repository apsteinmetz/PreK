...
```
Get census data
Income, population, population of school age children and census tract geometries.
```{r}
...
acs_table_pop <- "B01003_001"
acs_table_kid <- "B05009_002" #under 6
acs_table_inc <- "B19013_001" #median, last 12 months
# NYC county codes
...
# we can only use the zcta geography if we fetch the whole country!
# acs_raw <- get_acs(geography = "zcta",
#              variables = c(HouseholdIncome = acs_table_inc,
#                            kidsUnder3 = acs_table_kid,
#                            totpop = acs_table_pop))

# Let's opt for census tracts instead so we can just get NY
# Choose wide rather than tidy so we don't fetch multiple copies of spatial data.
acs_raw <- get_acs(geography = "tract",
                   variables = c(HouseholdIncome = acs_table_inc,
                                 kidsUnder3 = acs_table_kid,
                                 totpop = acs_table_pop),
                   geometry = TRUE,
...
select_if(acs_raw,function(x)str_detect(x,"(.+)E$"))

# filter for just NYC zip codes
income <- income_raw %>% 
  rename(zip=GEOID) %>% 
  select(zip,estimate) %>% 
  rename(HouseholdIncome=estimate) %>%
  right_join(nyc_zips) %>% 
  na.omit() %>% 
  select(zip,HouseholdIncome)
```
