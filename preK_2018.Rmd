...
all_data_2018 <- all_data_2018 %>% 
  mutate(combo = abs(seatsQuantile - incomeQuantile))
         ...
         #recode as numeric so we can do a continuous color scale
levels(all_data_2018$combo) <- 1:9
#collapse since we don't have an opinion about whether
#"low many" is better than  "high few"
alla
tmp
all_data_2018 <- all_data_2018 %>% 
  mutate(combo = as.integer(combo))


all_data_2018 %>%
  ggplot(aes(fill = seatsBin, color = seatsBin)) +
  geom_sf()+
