...
To create a bivariate map we add a new factor that is the character combination of the income bin and the seat count bin.  Then we are done monkeying with our data set.  We can look at the zip code for Amazon's new NYC HQ it illustrate what we've got.
...
We saw how easy it is to make a map with `mapview.`  Going forward we want more control over layers and other map features so we'll directly access the package `mapview` wraps, which is `leaflet.`
